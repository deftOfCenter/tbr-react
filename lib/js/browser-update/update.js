var $buo=function(op,test){var jsv=15;var n=window.navigator,b;this.op=op||{};this.op.l=op.l||(n.languages?n.languages[0]:null)||n.language||n.browserLanguage||n.userLanguage||document.documentElement.getAttribute("lang")||"en";var ll=this.op.l.substr(0,2);this.op.vsakt={i:11,f:34,o:12.1,s:8,n:20,c:39};this.op.vsdefault={i:9,f:30,o:12,s:5.2,n:12,c:35};this.op.vsmin={i:7,f:5,o:12,s:5.1,n:10};var myvs=op.vs||{};this.op.vs=op.vs||this.op.vsdefault;for(b in this.op.vsakt){if(this.op.vs[b]>=this.op.vsakt[b])
...
this.op.test=true;if(op.exp||(ll==="en"&&!this.op.test&&Math.round(Math.random()*200)<1)){var e=document.createElement("script");e.setAttribute("type","text/javascript");e.setAttribute("src","//browser-update.org/updatex.js");document.body.appendChild(e);return;}
function getBrowser(){var n,v,t,ua=navigator.userAgent;var names={i:'Internet Explorer',f:'Firefox',o:'Opera',s:'Apple Safari',n:'Netscape Navigator',c:"Chrome",x:"Other"};if(/bot|googlebot|facebook|slurp|wii|silk|blackberry|maxthon|maxton|mediapartners|dolfin|dolphin|adsbot|silk|android|phone|bingbot|google web preview|like firefox|chromeframe|seamonkey|opera mini|min|meego|netfront|moblin|maemo|arora|camino|flot|k-meleon|fennec|kazehakase|galeon|android|mobile|iphone|ipod|ipad|epiphany|konqueror|rekonq|symbian|webos|coolnovo|blackberry|bb10|RIM|PlayBook|PaleMoon|QupZilla/i.test(ua))n="x";else if(/Trident.*rv:(\d+\.\d+)/i.test(ua))n="i";else if(/Trident.(\d+\.\d+)/i.test(ua))n="io";else if(/MSIE.(\d+\.\d+)/i.test(ua))n="i";else if(/OPR.(\d+\.\d+)/i.test(ua))n="o";else if(/Chrome.(\d+\.\d+)/i.test(ua))n="c";else if(/Firefox.(\d+\.\d+)/i.test(ua))n="f";else if(/Version.(\d+.\d+).{0,10}Safari/i.test(ua))n="s";else if(/Safari.(\d+)/i.test(ua))n="so";else if(/Opera.*Version.(\d+\.\d+)/i.test(ua))n="o";else if(/Opera.(\d+\.?\d+)/i.test(ua))n="o";else if(/Netscape.(\d+)/i.test(ua))n="n";else return{n:"x",v:0,t:names[n]};var v=parseFloat(RegExp.$1);var donotnotify=false;if(/windows.nt.5.0|windows.nt.4.0|windows.98|os x 10.4|os x 10.5|os x 10.3|os x 10.2/.test(ua))donotnotify="oldOS";if(n=="f"&&Math.round(v)==24)
...
return;if(!this.op.test&&Math.round(Math.random()*100)<1){var i=new Image();i.src="//browser-update.org/viewcount.php?n="+this.op.browser.n+"&v="+this.op.browser.v+"&p="+escape(this.op.pageurl)+"&jsv="+jsv+"&inv="+this.op.v+"&vs="+myvs.i+","+myvs.f+","+myvs.o+","+myvs.s;}
...
t='Votre navigateur (%s) est <b>pÃ©rimÃ©</b>. Il contient des <b>failles de sÃ©curitÃ©</b> et pourrait <b>ne pas afficher certaines fonctionalitÃ©s</b> des sites internet rÃ©cents. <a%s>DÃ©couvrez comment mettre votre navigateur Ã  jour</a>';else if(ll=="da")
...
t='Nettleseren din (%s) er <b>utdatert</b>. Den har kjente <b>sikkerhetshull</b> og <b>kan ikke vise alle funksjonene</b> pÃ¥ denne og andre websider. <a%s>LÃ¦r hvordan du kan oppdatere din nettleser</a>';else if(ll=="zh")
...
t='Î‘Ï…Ï„ÏŒÏ‚ Î¿ Î¹ÏƒÏ„ÏŒÏ„Î¿Ï€Î¿Ï‚ ÏƒÎ±Ï‚ Ï…Ï€ÎµÎ½Î¸Ï…Î¼Î¯Î¶ÎµÎ¹: ÎŸ Ï†Ï…Î»Î»Î¿Î¼ÎµÏ„ÏÎ·Ï„Î®Ï‚ ÏƒÎ±Ï‚ (%s) ÎµÎ¯Î½Î±Î¹ <b>Ï€Î±ÏÏ‰Ï‡Î·Î¼Î­Î½Î¿Ï‚</b>.\
	<a%s>Î•Î½Î·Î¼ÎµÏÏŽÏƒÏ„Îµ Ï„Î¿ Ï€ÏÏŒÎ³ÏÎ±Î¼Î¼Î± Ï€ÎµÏÎ¹Î®Î³Î·ÏƒÎ®Ï‚ ÏƒÎ±Ï‚</a> Î³Î¹Î± Î¼ÎµÎ³Î±Î»ÏÏ„ÎµÏÎ· Î±ÏƒÏ†Î¬Î»ÎµÎ¹Î± ÎºÎ±Î¹ Î¬Î½ÎµÏƒÎ· ÏƒÎµ Î±Ï…Ï„Î®Î½ Ï„Î·Î½ Î¹ÏƒÏ„Î¿ÏƒÎµÎ»Î¯Î´Î±.';if(op.text)
...
var hm=document.getElementsByTagName("html")[0]||document.body;this.op.bodymt=hm.style.marginTop;hm.style.marginTop=(div.clientHeight)+"px";(function(me){document.getElementById("buorgclose").onclick=function(e){var e=e||window.event;if(e.stopPropagation)e.stopPropagation();else e.cancelBubble=true;me.op.div.style.display="none";hm.style.marginTop=me.op.bodymt;return true;};})(me);op.onshow(this.op);};var $buoop=$buoop||{};$bu=$buo($buoop);
